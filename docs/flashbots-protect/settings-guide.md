...
If `auctionTimeout=T`, where `T` is the time in milliseconds, is specified, and a transaction arrives close to the end of the current slot, it will be scheduled for the next slot.
